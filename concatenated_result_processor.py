...
def generate_clip_list(go_pro_folder):
    print('Getting clip list...')
    clip_list = []
    for root, dirs, files in os.walk(go_pro_folder):
        for file in files:
            if 'MP4' in file:
...
def generate_concatenaded_file(go_pro_folder, result):
    print('Generating concatenated result...')
    clip_list = generate_clip_list(go_pro_folder)
